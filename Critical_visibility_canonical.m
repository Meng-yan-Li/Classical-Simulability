function [CV, Tau_value, c_value, Pi_value, Status] = Critical_visibility_canonical(rho, d, opts, ell)
%
% Functionality:
%   Compute the critical visibility for a collection of input quantum states
%   using a canonical lift SDP hierarchy. The routine builds and solves a 
%   semidefinite program that searches for the largest scalar
%   `v` in [0,1] such that v*rho{x} + (1-v)/d * I admits the required
%   decomposition under classical strategy constraints.
%
% Inputs:
...
%   Tau_value- cell(d, d^n) numeric array containing evaluated Tau matrices
%   c_value  - numeric vector (length d^n) of probabilities `c(mu)`
%   Pi_value - cell array containing evaluated Pi blocks for each mu
%   Status   - solver information / exit status returned by YALMIP
%
% Dependencies and Requirements (requirements):
%   - YALMIP (https://yalmip.github.io/) to model the SDP
%   - A backend SDP solver (MOSEK recommended and used by default)
%
% Example:
%   rho_set = {rho1, rho2};
%   opts = struct('solver','mosek','verbose',1,'threads',4,'gap',1e-6);
%   [CV, Tau_value, c_value, Pi_value, Status] = Critical_visibility_canonical(rho_set, 3, opts, 2);
%
% Other notes:
%   - The function returns numeric values for the structural variables
%     (`Tau`, `Pi`, and `c`) after solving the SDP.
%   - Defaults are applied when fields of `opts` are missing.
%
% Author: Mengyan-Li
...
% Date: 2026-05-06
    ...
    Pi = cell(numTuples, numMu);
    for mu = 1:numMu
        for alpha = 1:numTuples
            % Structural complex Hermitian variable
            Pi{alpha,mu} = sdpvar(Djoint, Djoint, 'hermitian', 'complex');
        end
    end
    ...
    for mu = 1:numMu
        ...
        % Pi PSD + trace
        for alpha = 1:numTuples
            Constraints = [Constraints, Pi{alpha,mu} >= 0];
            Constraints = [Constraints, trace(Pi{alpha,mu}) == c(mu)];
        end
        ...
        for j = 1:ell
            for s = 1:d
                ...
                for kk = 1:numel(alpha_idx)
                    sumBlock = sumBlock + Pi{alpha_idx(kk), mu};
                end
            ...
            for b = a+1:ell
                ...
                for alpha = 1:numTuples
                    if tupleList(alpha, a) ~= tupleList(alpha, b)
                        Constraints = [Constraints, Kab * reshape(Pi{alpha,mu}, Djoint^2, 1) == 0];
                    end
                end
            end
        end
    ...
    % 取出 Tau 的数值
    Tau_value = cell(d, numMu);
    for mu = 1:numMu
        for m = 1:d
            Tau_value{m,mu} = value(Tau{m,mu});
        end
    end

    % 取出 Pi 的数值
    Pi_value = cell(numTuples, numMu);
    for mu = 1:numMu
        for alpha = 1:numTuples
            Pi_value{alpha,mu} = value(Pi{alpha,mu});
        end
    end
